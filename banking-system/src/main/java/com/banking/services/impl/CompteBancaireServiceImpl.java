...
public class CompteBancaireServiceImpl implements CompteBancaireService {
    ...
    @Autowired private EncryptionService        encryptionService; // ✅ ajout
    ...
    public CompteBancaire createAccount(Long clientId) {
        ...
        if (compteBancaireRepository.existsByClientId(client.getId())) {
            return compteBancaireRepository.findFirstByClientIdOrderByIdAsc(client.getId()).orElseThrow();
        }
    ...
    public CarteBancaire issueCard(String numCompte) {
        ...
        // ✅ DELETE direct via JPQL — bypass le cache JPA
        carteBancaireRepository.deleteByCompteBancaire(compte);
        carteBancaireRepository.flush();

        String cvvClair   = generateCVV();
        String cvvChiffre = encryptionService.encrypt(cvvClair);

        CarteBancaire carte = new CarteBancaire(
                generateCardNumber(), LocalDate.now().plusYears(3), cvvChiffre, compte);
        carte.setPlafondJournalier(500d);
        carte.setPlafondMensuel(2000d);

        CarteBancaire saved = carteBancaireRepository.save(carte);
    ...
    public CompteEpargne convertToSavingsAccount(String numCompte, BigDecimal premierMontant) {
        ...
        BigDecimal montant = (premierMontant != null && premierMontant.signum() >= 0) ? premierMontant : BigDecimal.ZERO;
    ...
    public CompteEpargne updateSavingsRate(String numCompte, BigDecimal nouveauTaux) {
        throw new UnsupportedOperationException("Le taux est géré dans Interet, pas dans CompteEpargne");
    }

    @Override
    public void capitalizeInterests(String numCompte) {
        throw new UnsupportedOperationException("La capitalisation est gérée dans InteretService");
    }

    @Override
    public BigDecimal calculateInterests(String numCompte) {
        throw new UnsupportedOperationException("Le calcul des intérêts est géré dans InteretService");
    }
    ...
    public boolean isSavingsAccount(String numCompte) {
        CompteBancaire compte = getCompteOrThrowByNum(numCompte);
        return compte instanceof CompteEpargne || compteEpargneRepository.existsByNumCompte(numCompte);
    }

    @Override @Transactional(readOnly = true)
    public BigDecimal getBalance(String numCompte) { return getCompteOrThrowByNum(numCompte).getBalance(); }

    @Override @Transactional(readOnly = true)
    public List<Operation> getAccountHistory(String numCompte, LocalDateTime debut, LocalDateTime fin) {
        CompteBancaire compte = getCompteOrThrowByNum(numCompte);
        return operationRepository.findByCompteBancaireAndDateOperationBetweenOrderByDateOperationDesc(compte, debut, fin);
    }

    @Override @Transactional(readOnly = true)
    public List<Operation> getLastOperations(String numCompte, int nombre) {
        CompteBancaire compte = getCompteOrThrowByNum(numCompte);
        return operationRepository.findByCompteBancaireOrderByDateOperationDesc(compte, PageRequest.of(0, nombre)).getContent();
    }
    ...
    public Map<String, BigDecimal> getAccountStatistics(String numCompte) {
        ...
        List<Operation> allOps = operationRepository
                .findByCompteBancaire_NumCompteAndDateOperationBetweenOrderByDateOperationDesc(numCompte, veryOld, now);
        BigDecimal depots = allOps.stream().filter(o -> o.getTypeOperation() == TypeOperation.DEPOT)
                .map(Operation::getMontant).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal retraits = allOps.stream().filter(o -> o.getTypeOperation() == TypeOperation.RETRAIT)
                .map(Operation::getMontant).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
        stats.put("totalDepots",   depots);
        stats.put("totalRetraits", retraits);
        List<Operation> recentOps = operationRepository
                .findByCompteBancaireAndDateOperationBetweenOrderByDateOperationDesc(compte, now.minusDays(30), now);
        BigDecimal mouvements = recentOps.stream().map(Operation::getMontant).filter(Objects::nonNull)
    ...
    @Override @Transactional(readOnly = true)
    public List<CompteBancaire> findByStatus(AccountStatus status) { return compteBancaireRepository.findByStatus(status); }

    @Override @Transactional(readOnly = true)
    public List<CompteBancaire> findAccountsWithLowBalance(BigDecimal threshold) {
        return compteBancaireRepository.findAll().stream().filter(c -> c.getBalance().compareTo(threshold) < 0).toList();
    }

    @Override @Transactional(readOnly = true)
    public List<CompteBancaire> findInactiveAccounts(int daysSinceLastActivity) {
        return compteBancaireRepository.findInactiveAccountsOlderThan(LocalDateTime.now().minusDays(daysSinceLastActivity));
    }
    ...
    public List<CompteBancaire> findAccountsNeedingAttention() {
        ...
        for (CarteBancaire carte : carteBancaireRepository.findExpiredCards(LocalDate.now())) {
            if (!accounts.contains(carte.getCompteBancaire())) accounts.add(carte.getCompteBancaire());
        }
        return accounts;
    }

    @Override @Transactional(readOnly = true)
    public boolean accountExists(String numCompte) { return compteBancaireRepository.existsByNumCompte(numCompte); }

    @Override @Transactional(readOnly = true)
    public boolean canPerformOperation(String numCompte, BigDecimal montant) {
        return compteBancaireRepository.findByNumCompte(numCompte)
                .map(cb -> cb.isActive() && cb.getBalance().compareTo(montant) >= 0).orElse(false);
    }
    ...
    public BigDecimal getRemainingDailyLimit(String numCompte) {
        ...
        if (carte == null || Objects.equals(carte.getPlafondJournalier(), 0d)) return BigDecimal.ZERO;
        List<Operation> todayOps = operationRepository
                ...
                .stream().filter(op -> op.getTypeOperation() == TypeOperation.RETRAIT).toList();
        BigDecimal total     = todayOps.stream().map(Operation::getMontant).filter(Objects::nonNull).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal remaining = BigDecimal.valueOf(carte.getPlafondJournalier()).subtract(total);
        return remaining.signum() < 0 ? BigDecimal.ZERO : remaining;
    }

    @Override public void sendLowBalanceAlert(String numCompte) {}
    @Override public void sendStatementNotification(String numCompte) {}
    @Override public List<String> getPendingNotifications(String numCompte) { return Collections.emptyList(); }
    ...
    private String generateCardNumber() { return generateAccountNumber(); }

    private String generateCVV() { return String.format("%03d", new Random().nextInt(1000)); }
